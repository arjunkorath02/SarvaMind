...
const formatAIResponse = (content: string): string => {
  ...
  const greetingPatterns = [/^Hello! I'm.*?(?=\n|$)/gim, /^Hi! I'm.*?(?=\n|$)/gim, /^I'm SarvaMind.*?(?=\n|$)/gim, /^I'm your.*?assistant.*?(?=\n|$)/gim];
...
const EnhancedChatInterface: React.FC<EnhancedChatInterfaceProps> = ({
  ...
  const getConversationHistory = (): string => {
    const recentMessages = messages.filter(msg => !msg.content.toLowerCase().includes("i'm sarvamind") && !msg.content.toLowerCase().includes("how can i help")).slice(-4);
  ...
  return <div className="flex h-screen bg-black relative overflow-hidden">
      {/* Hamburger Menu Button - Fixed positioning */}
      <Button onClick={() => setSidebarOpen(!sidebarOpen)} variant="ghost" size="sm" className="fixed top-4 left-4 z-50 text-muted-foreground hover:text-white glass-card glow-subtle md:hidden" title="Toggle sidebar">
      ...
      <div className="flex-1 flex flex-col relative bg-black min-w-0">
        ...
        <ScrollArea ref={scrollAreaRef} className="flex-1 p-4 pb-40">
          <div className="space-y-6 max-w-4xl mx-auto">
            {messages.map(message => <div key={message.id} className={`flex gap-3 ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                {message.sender === 'ai' && <div className="w-8 h-8 rounded-full overflow-hidden glow-subtle flex-shrink-0">
                    <img src="https://raw.githubusercontent.com/arjunkorath02/SarvaMindlogo/main/SarvaMind%20Logo.png" alt="SarvaMind" className="w-full h-full object-cover" />
                  </div>}
                
                <div className={`max-w-[85%] sm:max-w-[70%] ${message.sender === 'user' ? 'order-first' : ''}`}>
                  <div className={`glass-card rounded-2xl p-4 mb-6 ${message.sender === 'user' ? 'bg-gradient-to-r from-primary/20 to-accent/20 glow-subtle border-primary/30' : 'border-primary/20'}`}>
                    {message.sender === 'ai' && message.content.includes('<') ? <div className="text-white leading-relaxed break-words prose prose-invert max-w-none" dangerouslySetInnerHTML={{
                  ...
                  <div className="flex items-center justify-between px-2 mb-6">
                    ...
                    {message.sender === 'ai' && <div className="flex items-center gap-2 z-20 relative">
        ...
        <div className="fixed bottom-4 left-4 right-4 md:left-1/2 md:transform md:-translate-x-1/2 md:w-full md:max-w-4xl md:px-4 z-30">
          <div className="space-y-4">
            {/* Media Upload Area */}
            {showMediaUpload && <div className="backdrop-blur-xl bg-black/30 rounded-2xl p-4 border border-primary/30 shadow-2xl glow-subtle">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-white" title="Generate image">
